...
async function cargarCitas() {
  try {
    ...
    if (citas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="10" class="px-6 py-4 text-center text-gray-400">No hay citas registradas.</td></tr>';
      return;
    }
    citas.forEach((cita, index) => {
      const displayId =
        cita.client_sequence != null ? cita.client_sequence : index + 1;
      const nombre =
        cita.nombre_cliente ||
      ...
      const barbero =
        cita.barbero_nombre && cita.barbero_apellido
          ? `<a href="/perfil/${cita.barbero_id}" class="text-gray-200 hover:text-gray-400 underline">${cita.barbero_nombre} ${cita.barbero_apellido}</a>`
          : "-";
      ...
      tr.innerHTML = `
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200">${displayId}</td>
        ...
        year: "numeric",
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200">
                    ...
                    ${
                      cita.servicio_adicional_nombre
                        ? `<span class="text-gray-400"> + </span>${cita.servicio_adicional_nombre}`
                        : ""
                    }
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200" data-estado="${
                  cita.estado
                }">${estado}</td>
