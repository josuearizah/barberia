...
async function cargarCitas() {
  try {
    ...
    if (citas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="px-6 py-4 text-center text-gray-400">No hay citas registradas.</td></tr>';
      totalCitas.textContent = "0";
      return;
    }
    citas.forEach((cita) => {
      ...
      tr.className =
        cita.estado === "completado" ? "bg-gray-950 opacity-70" : "";
      tr.innerHTML = `
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200">
                    ${
                      cita.nombre_cliente && cita.nombre_cliente.trim()
                        ? nombre
                        : `<a href="/perfil/${cita.usuario_id}" class="text-gray-200 hover:text-gray-400 underline">${nombre}</a>`
                    }
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200">${
                  cita.telefono_cliente || "-"
                }</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200" data-fecha_creacion="${
                  cita.fecha_creacion
        ...
        year: "numeric",
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200" data-fecha_cita="${
                  cita.fecha_cita
        ...
        year: "numeric",
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200" data-hora="${
                  cita.hora
                }">${cita.hora_12h}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-200">${
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm" data-estado="${
                  cita.estado
                }">
                    <select class="estado-cita text-white bg-gray-900 py-1 px-1 rounded-md border border-gray-600 text-sm focus:ring-rose-500 focus:border-rose-500"
                        ...
                        <option value="confirmado" ${
                          cita.estado === "confirmado" ? "selected" : ""
                        ...
                        <option value="completado" ${
                          cita.estado === "completado" ? "selected" : ""
    ...
    document.querySelectorAll(".editar-cita").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document.getElementById("edit-barbero").value =
          btn.dataset.barberoId || "";
        document.getElementById("edit-fecha_cita").value =
          btn.dataset.fechaCita || "";
        document.getElementById("edit-hora").value = btn.dataset.hora || "";
        document.getElementById("edit-servicio").value =
          btn.dataset.servicioId || "";
    ...
    document.querySelectorAll(".eliminar-cita").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document.getElementById("eliminar-client-id-display").textContent =
          btn.dataset.clientCitaId;
...
async function cargarDropdowns() {
  try {
    ...
    barberoSelect.innerHTML =
      ...
      barberos
        .map(
          (b) => `<option value="${b.id}">${b.nombre} ${b.apellido}</option>`
        )
        .join("");
...
document
  .querySelectorAll(".cerrar-modal-editar")
  .forEach((b) =>
    b.addEventListener("click", () => cerrar("modal-editar-cita"))
  );
modalEditar?.addEventListener("click", (e) => {
  if (e.target.id === "modal-editar-cita") cerrar("modal-editar-cita");
});

document
  .getElementById("form-cita-editar")
...
// Eliminar cita
document
  .querySelectorAll(".cerrar-modal-eliminar")
  .forEach((b) =>
    b.addEventListener("click", () => cerrar("modal-eliminar-cita"))
  );
document
  .getElementById("modal-eliminar-cita")
  ?.addEventListener("click", (e) => {
    if (e.target.id === "modal-eliminar-cita") cerrar("modal-eliminar-cita");
  });

document
  .getElementById("form-cita-eliminar")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
  ...
  if (searchInput) {
    ...
    function updateCount() {
      const visibleRows = Array.from(
        tbody.querySelectorAll("tr:not(.hidden)")
    ...
    function applyFiltersAndSort() {
      ...
      rows.forEach((tr) => {
        ...
        if (filterDate) {
          const fechaCita =
            tr.querySelector("[data-fecha_cita]").dataset.fecha_cita;
      ...
      visibleRows.sort((a, b) => {
        const fechaA = new Date(
          a.querySelector("[data-fecha_cita]").dataset.fecha_cita
        );
        const fechaB = new Date(
          b.querySelector("[data-fecha_cita]").dataset.fecha_cita
        );
    ...
    closeFilterPopover.addEventListener("click", () =>
    ...
    orderToggle?.addEventListener("click", () => {
      ...
      orderToggle.querySelector("span").textContent = orderByDateDesc
    ...
    window.sortTable = function (field) {
      const th = Array.from(table.querySelectorAll("th")).find(
        (t) => t.onclick && t.onclick.toString().includes(`'${field}'`)
      );
      if (!th) return;
      ...
      const rows = Array.from(tbody.querySelectorAll("tr")).filter(
        (tr) =>
          !tr.querySelector("td[colspan]") && !tr.classList.contains("hidden")
      );
    ...
    function abrirModalPago(data, select) {
      ...
      (data.items || []).forEach((it) => {
        ...
        tr.innerHTML = `
                  <td class="py-1 pr-2">${
                    it.tipo === "principal" ? "Servicio" : "Adicional"
                  }: ${it.nombre || "-"}</td>
      ...
      if (elSaldo) {
        const saldoValor = Object.prototype.hasOwnProperty.call(data, "saldo")
          ? data.saldo
          : data.total;
    ...
    (function initPagoModalEvents() {
      ...
      async function crearPagoSiNecesario() {
        ...
        if (!resPago.ok)
          throw new Error(dataPago.error || "No se pudo registrar el pago");
      ...
      btnCerrar?.addEventListener("click", () => {
        cerrarModal();
        if (pagoContext.select)
          pagoContext.select.value = pagoContext.select.dataset.currentValue;
      });
    ...
    window.actualizarEstadoCita = async function (select) {
      ...
      try {
        const response = await fetch(
          `/api/citas/${select.dataset.citaId}/estado`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
