(() => {
    ...
    function aplicarFiltros() {
      ...
      filtrados = estilos
        ...
        .filter(e => termino ? normalizarTexto(e.nombre).includes(termino) : true);
      ...
      return `
        <article class="bg-white border border-gray-300 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition">
          <div class="relative h-[200px] md:h-[250px] bg-gray-700 overflow-hidden flex items-center justify-center">
            <img src="${img}" alt="${e.nombre}" class="max-w-full max-h-full object-contain object-center">
          </div>
    ...
    btnPrev.addEventListener('click', () => { pagina = Math.max(1, pagina - 1); render(); });
    btnNext.addEventListener('click', () => { pagina += 1; render(); });
